...
class Filemanager {
  ...
  public function getinfo() {
    ...
    $this->get_file_info();
     if(strtolower($this->item['filetype']) == 'jpg' || strtolower($this->item['filetype']) == 'jpeg' || strtolower($this->item['filetype']) == 'gif' || strtolower($this->item['filetype']) == 'png'){
     $MimeType = 'image';
     }
     else $MimeType = 'other';
  ...
  public function getfolder() {
    ...
    $current_path = rawurldecode($this->doc_root . $this->get['path']);
    //echo $current_path;
  ...
  $TmpFile = time();
  $DATA = array();
  $DATA['src']['type'] = $_REQUEST['image_type'];
  $DATA['src']['name'] = $_REQUEST['oldname'];
  $DATA['src']['path'] = $this->doc_root.str_replace($DATA['src']['name'],'',rawurldecode($_REQUEST['path']));
  $DATA['src']['width'] = $_REQUEST['image_width'];
  $DATA['src']['height'] = $_REQUEST['image_height'];
  $DATA['src']['crop_x'] = $_REQUEST['crop_x'];
  $DATA['src']['crop_y'] = $_REQUEST['crop_y'];
  $DATA['src']['crop_width'] = $_REQUEST['crop_width'];
  $DATA['src']['crop_height'] = $_REQUEST['crop_height'];
  
  $DATA['dst']['name'] = $_REQUEST['newname'];  
  $DATA['dst']['x1'] = ($DATA['src']['crop_x'] < 0) ? 0 : $DATA['src']['crop_x'];
  $DATA['dst']['x2'] = (($DATA['src']['crop_x'] + $DATA['src']['crop_width']) > $DATA['src']['width']) ? $DATA['src']['width'] : $DATA['src']['crop_x'] + $DATA['src']['crop_width'];
  $DATA['dst']['y1'] = ($DATA['src']['crop_y'] < 0) ? 0 : $DATA['src']['crop_y'];
  $DATA['dst']['y2'] = (($DATA['src']['crop_y'] + $DATA['src']['crop_height']) > $DATA['src']['height']) ? $DATA['src']['height'] : $DATA['src']['crop_y'] + $DATA['src']['crop_height'];
  $DATA['dst']['width'] = $DATA['dst']['x2'] - $DATA['dst']['x1'];
  $DATA['dst']['height'] = $DATA['dst']['y2'] - $DATA['dst']['y1'];
  
  
   if(file_exists($DATA['src']['path'].$DATA['dst']['name']) && $DATA['src']['name'] != $DATA['dst']['name'])$this->error(sprintf($this->lang('FILE_ALREADY_EXISTS'),$DATA['dst']['name']));
  
  $Tmp = getimagesize($DATA['src']['path'].$DATA['src']['name']);
 
   if($DATA['src']['type'] == 'image/jpeg')$ImgSrc = imagecreatefromjpeg($DATA['src']['path'].$DATA['src']['name']);
   if($DATA['src']['type'] == 'image/gif')$ImgSrc = imagecreatefromgif($DATA['src']['path'].$DATA['src']['name']);
   if($DATA['src']['type'] == 'image/png')$ImgSrc = imagecreatefrompng($DATA['src']['path'].$DATA['src']['name']);
   
  $ImgDstTmp = imagecreatetruecolor($DATA['src']['width'], $DATA['src']['height']);
  imagecopyresized ($ImgDstTmp, $ImgSrc , 0, 0, 0, 0, $DATA['src']['width'], $DATA['src']['height'], $Tmp[0], $Tmp[1]);
  $ImgDst = imagecreatetruecolor($DATA['dst']['width'], $DATA['dst']['height']);  
  imagecopy ($ImgDst, $ImgDstTmp, 0, 0, $DATA['dst']['x1'], $DATA['dst']['y1'], $DATA['src']['width'], $DATA['src']['height']);

  //header('Content-Type: '.$DATA['scr']['type']);
   if($DATA['src']['type'] == 'image/jpeg')imagejpeg($ImgDst,$DATA['src']['path'].$DATA['dst']['name'],100);
   if($DATA['src']['type'] == 'image/gif')imagegif($ImgDst,$DATA['src']['path'].$DATA['dst']['name']);
   if($DATA['src']['type'] == 'image/png')imagepng($ImgDst,$DATA['src']['path'].$DATA['dst']['name'],0);

  imagedestroy($ImgSrc);
  imagedestroy($ImgDst);
  imagedestroy($ImgDstTmp);

  $array = array(
  			'Error'=>"",
  			'Code'=>0,
  			//'Old Path' => $DATA['src']['path'].$DATA['src']['name'],
  			'Old Path' => $_REQUEST['path'],
  			'Old Name' => $DATA['src']['name'],
  			//'New Path' => $DATA['src']['path'].$DATA['dst']['name'],
  			'New Path' => str_replace($_REQUEST['oldname'],'',$_REQUEST['path']).$DATA['dst']['name'],
  			'New Name' => $DATA['dst']['name']
  );
  ...
  $DATA['code'] = -1;
  $DATA['error'] = '';
  $DATA['source'] = $_REQUEST['item'];
  
  $DATA['source-no-slash'] = (substr($DATA['source'],strlen($DATA['source'])-1) == "/") ? substr($DATA['source'],0,strlen($DATA['source'])-1) : $DATA['source'];
  $Tmp = explode("/",$DATA['source-no-slash']);
  $DATA['source-folder'] = $Tmp[sizeof($Tmp)-1];  
  $Tmp = array_slice($Tmp,0,sizeof($Tmp)-1);
  $DATA['source-path'] = implode("/",$Tmp).'/';
  $DATA['target-path'] = $_REQUEST['path'];
  $DATA['source-type'] = $_REQUEST['type'];
  
   if($DATA['source-path'] == $DATA['target-path'])$DATA['error'] = tr('MSG_UNABLE_TO_MOVE_WITHIN_SAME_LEVEL');
   if(strpos($DATA['target-path'],$DATA['source']) !== false) $DATA['error'] = tr('MSG_UNABLE_TO_MOVE_TO_CHILD_ITEM');
   ...
   $Path = $_SERVER['DOCUMENT_ROOT'];
    if($DATA['source-type'] == ''){
    copy_r( $Path.$DATA['source'], $Path.$DATA['target-path'].$DATA['source-folder'] );
    $this->unlinkRecursive($Path.$DATA['source']);
    }
    else {
    copy_r( $Path.$DATA['source'], $Path.$DATA['target-path'].$DATA['source-folder']);
    @unlink($Path.$DATA['source']);
    }

   }

  $array = array(
			'Parent'=>$DATA['target-path'],
...
global $MYVAR;
    ...
    $tmpIconFile = $MYVAR['ROOT'].'fm/'.$this->config['icons']['path'].strtolower($this->item['filetype']).'.png';
    $this->item['icon'] = (file_exists($tmpIconFile)) ? str_replace($MYVAR['ROOT'],$MYVAR['HTTP'],$tmpIconFile) : $MYVAR['HTTP'].'fm/'.$this->config['icons']['path'].'default.png';
    $this->item['filemtime'] = filemtime(urldecode($this->doc_root . $path));
    $this->item['filectime'] = filectime(urldecode($this->doc_root . $path));
    $this->item['preview'] = $this->config['icons']['path'] . $this->config['icons']['default'];

     if(is_dir($this->doc_root . $path)) {                                                                     //is dir
     $this->item['preview'] = $this->config['icons']['path'] . $this->config['icons']['directory'];
     }
     else{
     $this->item['properties']['Size'] = filesize(urldecode($this->doc_root . $path));
     $this->item['properties']['Date Modified'] = date($this->config['date'], $this->item['filemtime']);
     $this->item['properties']['Date Created'] = date($this->config['date'], $this->item['filectime']);
     
     
     //print_r($tmp);
      if(in_array(strtolower($this->item['filetype']),$this->config['images'])) {                          //is image
      $tmp = getimagesize(urldecode($this->doc_root . $path));
      $this->item['properties']['Height'] = $tmp[1];
      $this->item['properties']['Width'] = $tmp[0];
      $this->item['properties']['Type'] = $tmp['mime'];
     //$Tmp = getimagesize($_SERVER['DOCUMENT_ROOT'].'/'.$Image);
     //$ImageMimeType = 
     //var_dump($this->doc_root);
     //var_dump($path) ;
     //echo'<pre>';print_r($this->item);
      ...
      //$this->item['properties']['Type'] = $type;
      //$this->item['properties']['Size'] = filesize($this->doc_root . $path);
     ...
     //}
     //else {
     //$this->item['properties']['Height'] = 0;
     //$this->item['properties']['Width'] = 0; 	
     //$this->item['properties']['Size'] = filesize($this->doc_root . $path);
    // }
     }
    //$this->item['properties']['Date Modified'] = date($this->config['date'], $this->item['filemtime']);
  ...
  private function copyRecursive($dir,$target) {
    if(!$dh = @opendir($dir)) {
      return;
    }
    while (false !== ($obj = readdir($dh))) {
      ...
      if (!@copy($dir . '/' . $obj, $target)) {
        $this->copyRecursive($dir.'/'.$obj, $target);
      }
    }
  ...
  private function loadLanguageFile() {
    ...
    if(file_exists($_SERVER['DOCUMENT_ROOT'].'/my/scripts/languages/'.$lang.'.js')) {
      $stream = file_get_contents($_SERVER['DOCUMENT_ROOT'].'/my/scripts/languages/'.$lang.'.js');
  ...
  private function availableLanguages() {
    ...
    if ($handle = opendir($_SERVER['DOCUMENT_ROOT'].'/my/scripts/languages/')) {
      while (false !== ($file = readdir($handle))) {
        if ($file != "." && $file != "..") {
