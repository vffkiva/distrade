...
defaults['crop_x'] = 10;
defaults['crop_y'] = 10;
defaults['crop_width'] = 50;
defaults['crop_height'] = 50;
defaults['canvas_width'] = 670;
defaults['canvas_height'] = 470;

function autoResize(Action,value){
tmp = $('#container-img-dimensions').html().split('x');
defaults['image_width'] = (parseInt(tmp[0]) != '' && !isNaN(parseInt(tmp[0]))) ? parseInt(tmp[0]) : '';
defaults['image_height'] = (parseInt(tmp[1]) != '' && !isNaN(parseInt(tmp[1]))) ? parseInt(tmp[1]) : '';

var inputs = $(document).find('#container-img-resize-inputs input:text');
var $viewport = $('#container-iframe').contents().find('.jrac_viewport');
var $crop = $('#container-iframe').contents().find('.jrac_crop');
var $image = $viewport.find('img');
var $slider = $('#container-iframe').contents().find('.jrac_zoom_slider');

var Values = [];
var New = [];

Values['crop_x'] = parseInt($(inputs[0]).val());
Values['crop_y'] = parseInt($(inputs[1]).val());
Values['crop_width'] = parseInt($(inputs[2]).val());
Values['crop_height'] = parseInt($(inputs[3]).val());
Values['image_width'] = parseInt($($image).css('width'))
Values['image_height'] = parseInt($($image).css('height'))
Values['image_x'] = parseInt($($image).css('left'));
Values['image_y'] = parseInt($($image).css('top'));
Values['canvas_width'] = parseInt($(inputs[6]).val());
Values['canvas_height'] = parseInt($(inputs[7]).val());

 if(Action == 'crop2image'){
 New['crop_x1'] = (Values['image_x'] < 0) ? 0 : Values['image_x'];
 New['crop_x2'] = ((Values['image_x'] + Values['image_width']) > Values['canvas_width']) ? Values['canvas_width'] : Values['image_x'] + Values['image_width'];
 New['crop_y1'] = (Values['image_y'] < 0) ? 0 : Values['image_y'];
 New['crop_y2'] = ((Values['image_y'] + Values['image_height']) > Values['canvas_height']) ? Values['canvas_height'] : Values['image_y'] + Values['image_height'];  
 New['crop_width'] = New['crop_x2'] - New['crop_x1'];
 New['crop_height'] = New['crop_y2'] - New['crop_y1'];
 $crop.css({'width' : New['crop_width'], 'height' : New['crop_height'], 'left' : New['crop_x1'], 'top' : New['crop_y1']});
 }

 if(Action == 'canvas2image'){
 New['canvas_width'] = Values['image_width'];
 New['canvas_height'] = Values['image_height'];
 $viewport.css({'width' : New['canvas_width'], 'height' : New['canvas_height']});
 $image.css({'top' : 0, 'left' : 0});
 $slider.css({'width' : New['canvas_width']});
 $crop.css({'left' : defaults['crop_x'], 'top' : defaults['crop_y'], 'width' : defaults['crop_width'], 'height' : defaults['crop_height']});
 }

 if(Action == 'image2crop'){
 $image.css({'top' : parseInt($($crop).css('top')), 'left' : parseInt($($crop).css('left')), 'width' : Values['crop_width'], 'height' : Values['crop_height']});
 }

 if(Action == 'image2canvas'){
 $image.css({'top' : 0, 'left' : 0, 'width' : Values['canvas_width'], 'height' : Values['canvas_height']});
 }
 
 if(Action == 'reset'){
 $image.css({'top' : 0, 'left' : 0, 'width' : defaults['image_width'], 'height' : defaults['image_height']});
 $viewport.css({'width' : defaults['canvas_width'], 'height' : defaults['canvas_height']});
 $slider.css({'width' : defaults['canvas_width']});
 $crop.css({'left' : defaults['crop_x'], 'top' : defaults['crop_y'], 'width' : defaults['crop_width'], 'height' : defaults['crop_height']});
 }
 ...
 Tmp = value.split('x');
  if(value!=''){
  $crop.css({'width' : Tmp[0], 'height' : Tmp[1]});
  }
 }

 $(inputs[0]).val(parseInt($crop.css('left')));
 $(inputs[1]).val(parseInt($crop.css('top')));
 $(inputs[2]).val(parseInt($crop.css('width')));
 $(inputs[3]).val(parseInt($crop.css('height')));
 $(inputs[4]).val(parseInt($image.css('width')));
 $(inputs[5]).val(parseInt($image.css('height')));
 $(inputs[6]).val(parseInt($viewport.css('width')));
 $(inputs[7]).val(parseInt($viewport.css('height')));
}
...
var capabilities = new Array('select', 'download', 'rename', 'delete', 'move');

// Get localized messages from file 
// through culture var or from URL
//if($.urlParam('langCode') != 0 && file_exists ('/my/scripts/languages/'  + $.urlParam('langCode') + '.js')) culture = $.urlParam('langCode');

culture = (typeof langCode == 'undefined' || langCode == '') ? 'en' : langCode;
var lg = [];
$.ajax({
  url: '/my/scripts/languages/'  + culture + '.js',
  async: false,
  dataType: 'json',
...
var bindToolbar = function(data){
	...
	if (!has_capability(data, 'resize')) {
		...
		$('#fileinfo').find('button#resize').click(function(){
		var inputs = $(document).find('#container-img-resize-inputs input:text');
		resizeImage(data,inputs);
...
var selectItem = function(data){
	...
	if(window.opener){
		...
		//	}		
		//}
		if($.urlParam('action') == 'addToGallery'){
		window.opener.gallerySetPicture(data);
		}
...
var resizeImage = function(data,inputs){
	var msg = lg.msg_resize_image+':<br/>';
	var Values = [];
	Values['crop_x'] = parseInt($(inputs[0]).val());
	Values['crop_y'] = parseInt($(inputs[1]).val());
	Values['crop_width'] = parseInt($(inputs[2]).val());
	Values['crop_height'] = parseInt($(inputs[3]).val());
	Values['image_width'] = parseInt($(inputs[4]).val());
	Values['image_height'] = parseInt($(inputs[5]).val());
	var Error = '';

	var makeResize = function(){
	var givenName = nameFormat(rname);
	var params = '&crop_x='+Values['crop_x']+'&crop_y='+Values['crop_y']+'&crop_width='+Values['crop_width']+'&crop_height='+Values['crop_height']+'&image_width='+Values['image_width']+'&image_height='+Values['image_height']+'&image_type='+data['Properties']['Type'];
	var connectString = fileConnector + '?mode=resize&oldname='+data['Filename']+'&newname='+givenName+'&path=' + encodeURIComponent(data['Path'])+params;	
		$.ajax({
		type: 'GET',
		url: connectString,
		dataType: 'json',
		async: false,
			success: function(result){
			var newPath = result['New Path'].replace(result['New Name'],'');
			var newName = result['New Name'];
			addNode(newPath, newName);
			}
		});
	}
 
	var checkParams = function(v,m){
		if(v != 1) return false;
		rname = m.children('#rname').val();
		oldName = m.children('#oldname').val();
			if(rname != ''){
				if(oldName == rname){
			 		$.prompt(lg.msg_overwrite_file, {
			 	 		buttons: [{title: lg.no, value: false},{title: lg.yes, value: true}],
			 	 		submit: function(v,m,f){
			 				if(v == true){makeResize();}	
						}
					});
				}
				else makeResize();
 		...
 		if( Values['crop_x'] > Values['image_width'] || (Values['crop_x']+Values['crop_width']) < 0 || Values['crop_y'] > Values['image_height'] || (Values['crop_y']+Values['crop_height']) < 0)Error = 'no common area';
 		if(Error != ''){$.prompt(lg.msg_no_common_area,{ buttons: { OK: false} });}
 		if(Error == ''){
 		msg += lg.crop_x+': '+Values['crop_x']+'<br/>'+lg.crop_y+': '+Values['crop_y']+'<br/>';
		msg += lg.crop_width+': '+Values['crop_width']+'<br/>'+lg.crop_height+': '+Values['crop_height']+'<br/>';
		msg += lg.image_width+': '+Values['image_width']+'<br/>'+lg.image_height+': '+Values['image_height']+'<br/>';
		msg += '<input id="rname" name="rname" type="text" value="resized_' + data['Filename'] + '" />';
		msg += '<input id="oldname" name="oldname" type="hidden" value="' + data['Filename'] + '" />';
		var btns = {}; 
		btns[lg.resize] = true; 
		btns[lg.cancel] = false; 
 			$.prompt(msg, {callback: checkParams, buttons: btns});
 		}
}
...
var moveItem = function(data){
	...
	var doMove = function(v, m){
		if(v != 1) return false;	
		item = m.children('#move-selected-item').html();
		path = m.children('#move-selected-path').html();
		if(item == '' || path == '') return false;
		var connectString = fileConnector + '?mode=move&item='+encodeURIComponent(item)+'&path=' + encodeURIComponent(path)+'&type='+data['File Type'];
			$.ajax({
			type: 'GET',
			url: connectString,
			dataType: 'json',
			async: false,
			success: function(result){
					if(result['Error'] != '')$.prompt(result['Error']);
					if(result['Code'] == 0){
					//addFolder(result['Parent'], result['Name']);
					openTargetFolder(result['Parent']);
					getFolderInfo(result['Parent']);
...
var html = '';
html += lg.move_selected_item+': <span id="move-selected-item">'+data['Path']+'</span><br/><br/>to:<span id="move-selected-path"><!----></span><br/><div id="move-filetree" style="overflow:auto;height:170px;background:rgb(255,255,255); border:1px solid rgb(200,200,200);">Tree</div>';
var btns = {}; 
btns[lg.yes] = true; 
btns[lg.no] = false; 
...
$("#move-filetree").fileTree({
	root: '/files/',
	multiFolder: false,
	datafunc: my1populateFileTree,
	folderCallback: function(path){
		var thisNode = $('#move-filetree').find('a[rel="' + path + '"]');
		  if($(thisNode).parent().hasClass('collapsed'))$("#move-selected-path").html('');
		  else $("#move-selected-path").html(path);
		},
	after: function(data){}
	}, function(file){}
	);
...
var updateNode = function(oldPath, newPath, newName){
	console.log(oldPath+' '+newPath+' '+newName);
...
var addFolder = function(parent, name){
	var newNode = '<li class="directory collapsed"><a rel="' + parent + name + '/" class=" cap_select cap_rename cap_delete cap_move" href="#">' + name + '</a><ul class="jqueryFileTree" style="display: block;"></ul></li>';
...
var openTargetFolder = function(path){
//data = '<ul class="jqueryFileTree" style="display: none;"><li class="directory collapsed"><a href="#" class=" cap_select cap_rename cap_delete cap_move" rel="/files/TEST06/123/">123</a></li></ul>';
//$('#filetree').fileTree.folderCallback = showData(data);
//var a = $('#filetree').find('a[rel="' + path + '"]').parents().find('li[class="directory collapsed"]');
//var a = $('#filetree').find('a[rel="' + path + '"]').parents();
//var a = $('#filetree').find('a[rel="' + path + '"]');
//console.log(path);
//console.log($(a));
}
...
var getFileInfo = function(file){
	...
	var toolbar = '<form id="toolbar" method="post" action="">';
	...
	toolbar += '<button id="download" name="download" type="button" value="Download">' + lg.download + '</button>';
	 ...
	 //if(browseOnly != true) toolbar += '<button id="resize" name="resize" type="button" value="Resize">' + lg.resize + '</button>';
	...
	'<tr>'+
	'<td class="row-left" id="container-fileinfo"><!----></td>'+
	'<td class="row-right"><table class="table-right"><tr><td class="row-top" id="container-toolbar">'+toolbar+'</td></tr><tr><td class="row-bottom" id="container-image-resize"><!----></td></tr></table></td>'+
	'</tr></table>';
	...
	$.getJSON(fileConnector + '?mode=getinfo&path=' + encodeURIComponent(file) + '&time=' + d.getMilliseconds(), function(data){
		if(data['Code'] == 0){
			...
			var properties = '';
			properties += '<img src="'+data['Icon']+'"><br/>';
			properties += '<dt>' + lg.filename + '</dt><dd>' + data['Filename'] + '</dd>';
			properties += '<dt>' + lg.filetype + '</dt><dd>' + data['Mime Type'] + '</dd>'; 
			
			 if(data['Properties']['Width'] && data['Properties']['Width'] != '') properties += '<dt>' + lg.dimensions + '</dt><dd id="container-img-dimensions">' + data['Properties']['Width'] + 'x' + data['Properties']['Height'] + '</dd>';
			 ...
			 if(data['Mime Type'] == 'image'){
/*
			 properties += '<div id="container-img-resize-inputs">';
			 properties += '<dl><dt>'+lg.crop_x+'</dt><dd><input type="text" class="input-element"/></dd><dt>'+lg.crop_y+'</dt><dd><input type="text" class="input-element" /></dd><dt>'+lg.crop_width+'</dt><dd><input type="text" class="input-element" /></dd><dt>'+lg.crop_height+'</dt><dd><input type="text" class="input-element" /></dd><dt>'+lg.image_width+'</dt><dd><input type="text" class="input-element" /></dd><dt>'+lg.image_height+'</dt><dd><input type="text" class="input-element" /></dd><dt class="d-hidden">'+lg.lock_proportion+'</dt><dd class="d-hidden"><input type="checkbox"/></dd><dt>'+lg.canvas_width+'</dt><dd><input type="text" class="input-element" /></dd><dt>'+lg.canvas_height+'</dt><dd><input type="text" class="input-element" /></dd></dl>';
			 properties += '<dl><dt>'+lg.crop_presets+'</dt><dd><select id="cropPresets" onChange="autoResize(\'update-crop\',this.value);"><option value=""><!----></option><option value="497x264">497x264</option></select></dd></dl>';
			 properties += '</div>';
			 
			 properties += '<div id="container-img-resize-icons">';
			 properties += '<a id="crop2image" onClick="autoResize(\'crop2image\');" title="'+lg.crop2image+'"><!----></a>';
			 properties += '<a id="image2crop" onClick="autoResize(\'image2crop\');" title="'+lg.image2crop+'"><!----></a>';
			 properties += '<a id="canvas2image" onClick="autoResize(\'canvas2image\');" title="'+lg.canvas2image+'"><!----></a>';
			 properties += '<a id="image2canvas" onClick="autoResize(\'image2canvas\');" title="'+lg.image2canvas+'"><!----></a>';
			 properties += '<a id="reset" onClick="autoResize(\'reset\');" title="'+lg.reset+'"><!----></a>';
			 properties += '</div>';
*/			 
			 $('#container-image-resize').html('<iframe src="resizer/index.html?image='+encodeURIComponent(file)+'" width="1000" height="700" frameborder="0" id="container-iframe"/>').show();
			 }
			 else {
			 $('#container-image-resize').html('<!---->').hide();
			 }
			 
		        $('#container-fileinfo').html('<dl>'+properties+'</dl>');
...
var my1populateFileTree = function(path, callback){
	var d = new Date();
